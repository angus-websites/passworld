<div x-data="{text_to_copy: '{{$assword}}', text_copied: false}">
    ...
    <x-button-group class="mt-5 sm:mt-10 justify-center">
      <!--Left button-->
      <x-button @click.prevent="window.navigator.clipboard.writeText(text_to_copy);text_copied=true;setTimeout(() => text_copied = false, 3000)" class="btn-lg">
        <!--Refresh Icon-->
      ...
      <!--Right button-->
      <x-button wire:click="refreshAss" class="btn-secondary btn-lg space-x-1 mt-5" id="generate">
        <!--Generate icon -->
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <!-- Copied -->
    <div x-show="text_copied" class="p-2 text-center my-10"> 
        Copied to clipboard!
    </div>
